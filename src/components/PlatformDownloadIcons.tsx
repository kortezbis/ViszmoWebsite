...
/** Official Apple logo mark for macOS download CTAs. */
export function AppleLogoIcon({ className = 'btn-svg', size }: IconProps) {
...
/** Windows four-tile logo for Windows download CTAs. */
export function WindowsTileIcon({ className = 'btn-svg', size }: IconProps) {
    const dim = size ?? undefined;
    return (
        <svg
            className={className}
...
export function PlatformDownloadIcon({
    isApple,
    className = 'btn-svg',
    size,
}: IconProps & { isApple: boolean }) {
    return isApple ? (
        <AppleLogoIcon className={className} size={size} />
    ) : (
        <WindowsTileIcon className={className} size={size} />
    );
}

/** Lucide-style icon slot for dashboard action cards. */
export function AppleLogoActionIcon({
    size = 24,
    className = '',
}: {
    size?: number;
    ...
    return <AppleLogoIcon className={className} size={size} />;
}
